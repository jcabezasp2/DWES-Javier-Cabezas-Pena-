...
function OpcionesSelect(datos :any) :void {
    ...
    const select :HTMLSelectElement = document.querySelector('#titular');
...
async function buscarDatos(event :Event) :Promise<void>{
    event.preventDefault();

    let idUsuario : HTMLInputElement = document.querySelector('#titular');
    let cuenta : string = idUsuario.value;
...
function mostrarDatos(datos :any) :void{
    ...
    let div :HTMLDivElement = document.querySelector('#anadirMovimiento');
    div.style.display = 'block';
    }
}

function anadirMovimiento(evento :Event) :void{

    evento.preventDefault();
    const error :HTMLParagraphElement = document.querySelector('#cantidad-error');
    const inputCantidad :HTMLInputElement = document.querySelector('#cantidad');
    const inputIngreso :HTMLInputElement = document.querySelector('#ingreso');
    const inputReintegro :HTMLInputElement = document.querySelector('#reintegro');

    let cantidad :number = inputCantidad.valueAsNumber;
    let tipo :string = inputIngreso.checked? inputIngreso.value : inputReintegro.value;
...
function enviarMovimiento(cantidad :number, tipo :string, codigoCuenta :string){
    ...
    if(window.XMLHttpRequest) {
        ...
        ajax.onreadystatechange=function(){
            if(ajax.readyState == 4) {
                if(ajax.status == 200) {
                    let objetivo :HTMLDivElement = document.querySelector('#resultado'); 
                    objetivo.innerText = '';
